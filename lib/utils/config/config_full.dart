...
///
/// Port of openneomclaw/src/utils/config.ts (1825 LOC).
...
class SerializedStructuredHistoryEntry {
  ...
  Map<String, dynamic> toJson() => {
        ...
        if (pastedContents != null)
          'pastedContents':
              pastedContents!.map((k, v) => MapEntry(k.toString(), v.toJson())),
...
class SkillUsageEntry {
  ...
  Map<String, dynamic> toJson() =>
      {'usageCount': usageCount, 'lastUsedAt': lastUsedAt};
}
...
class NeomClawHints {
  ...
  factory NeomClawHints.fromJson(Map<String, dynamic> json) =>
      NeomClawHints(
...
class ProjectConfig {
  ...
  static ProjectConfig defaultConfig() => ProjectConfig(
  ...
  factory ProjectConfig.fromJson(Map<String, dynamic> json) {
    ...
    return ProjectConfig(
      ...
      enabledMcpjsonServers:
          (json['enabledMcpjsonServers'] as List?)?.cast<String>(),
      disabledMcpjsonServers:
          (json['disabledMcpjsonServers'] as List?)?.cast<String>(),
      enableAllProjectMcpServers: json['enableAllProjectMcpServers'] as bool?,
      disabledMcpServers:
          (json['disabledMcpServers'] as List?)?.cast<String>(),
      enabledMcpServers:
          (json['enabledMcpServers'] as List?)?.cast<String>(),
      activeWorktreeSession:
  ...
  Map<String, dynamic> toJson() => {
        ...
        if (lastModelUsage != null)
          'lastModelUsage':
              lastModelUsage!.map((k, v) => MapEntry(k, v.toJson())),
...
class GlobalConfig {
  ...
  factory GlobalConfig.fromJson(Map<String, dynamic> json) {
    ...
    return GlobalConfig(
      ...
      tipsHistory: (json['tipsHistory'] as Map<String, dynamic>?)
              ?.map((k, v) => MapEntry(k, (v as num).toInt())) ??
          {},
...
const List<String> globalConfigKeys = [
  'apiKeyHelper', 'installMethod', 'autoUpdates',
  'autoUpdatesProtectedForNative', 'theme', 'verbose',
  'preferredNotifChannel', 'shiftEnterKeyBindingInstalled', 'editorMode',
  'hasUsedBackslashReturn', 'autoCompactEnabled', 'showTurnDuration',
  'diffTool', 'env', 'tipsHistory', 'todoFeatureEnabled',
  'showExpandedTodos', 'messageIdleNotifThresholdMs', 'autoConnectIde',
  'autoInstallIdeExtension', 'fileCheckpointingEnabled',
  'terminalProgressBarEnabled', 'showStatusInTerminalTab',
  'taskCompleteNotifEnabled', 'inputNeededNotifEnabled',
  'agentPushNotifEnabled', 'respectGitignore',
  'neomClawInChromeDefaultEnabled', 'hasCompletedNeomClawInChromeOnboarding',
  'lspRecommendationDisabled', 'copyFullResponse', 'copyOnSelect',
...
class ConfigController extends SintController {
  ...
  String getProjectPathForConfig() {
    ...
    _projectPath =
        _normalizePathForConfigKey(gitRoot ?? p.canonicalize(cwd));
    return _projectPath!;
  }
  ...
  void saveCurrentProjectConfig(
    ...
    final didWrite = _saveConfigWithLock((current) {
      final currentPc = current.projects?[path] ?? ProjectConfig.defaultConfig();
  ...
  bool _computeTrustDialogAccepted({
    required bool sessionTrustAccepted,
  }) {
  ...
  String getMemoryPath(MemoryType memoryType, {String? managedFilePath}) {
    return switch (memoryType) {
      ...
      MemoryType.managed => p.join(
          managedFilePath ?? p.join(configHomeDir, 'managed'), 'NEOMCLAW.md'),
  ...
  GlobalConfig _readConfigFromDisk({bool throwOnInvalid = false}) {
    try {
      ...
      try {
        ...
        if (throwOnInvalid) throw ConfigParseError(e.toString(), configFilePath);
  ...
  bool _wouldLoseAuthState(GlobalConfig fresh) {
    ...
    final lostOnboarding = cached.hasCompletedOnboarding == true &&
