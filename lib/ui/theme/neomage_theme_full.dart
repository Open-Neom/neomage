...
///
/// Mirrors the semantic tokens found in Neomage's terminal theme, adapted
/// for Flutter's widget-based rendering.
class NeomageColorScheme {
  const NeomageColorScheme({
  ...
  factory NeomageColorScheme.fromMap(Map<String, dynamic> map) {
...
/// Typography definitions for Neomage, including monospaced variants for code
/// and terminal output.
class NeomageTextTheme {
  const NeomageTextTheme({
  ...
  factory NeomageTextTheme.defaults({Color color = Colors.white}) {
...
class NeomageComponentTheme {
  const NeomageComponentTheme({
...
/// A complete Neomage theme containing colours, typography, component sizes, and
...
class NeomageTheme {
  ...
  final NeomageColorScheme colors;
  final NeomageTextTheme textTheme;
  final NeomageComponentTheme componentTheme;
  ...
  factory NeomageTheme.fromMap(Map<String, dynamic> m) {
    return NeomageTheme(
      info: ThemeInfo(
      ...
      colors: NeomageColorScheme.fromMap(m['colors'] as Map<String, dynamic>),
      textTheme: NeomageTextTheme.defaults(), // text is regenerated from defaults
      componentTheme: const NeomageComponentTheme(),
...
class NeomageThemes {
  ...
  /// Deep navy / purple — the default Neomage dark theme.
  static final NeomageTheme darkDefault = NeomageTheme(
    ...
    colors: const NeomageColorScheme(
    ...
    textTheme: NeomageTextTheme.defaults(color: const Color(0xFFE6EDF3)),
    componentTheme: const NeomageComponentTheme(),
  ...
  static final NeomageTheme darkMonokai = NeomageTheme(
    info: const ThemeInfo(
      name: 'dark-monokai',
      displayName: 'Monokai',
      isDark: true,
    ...
    textTheme: NeomageTextTheme.defaults(color: const Color(0xFFF8F8F2)),
    componentTheme: const NeomageComponentTheme(),
    syntaxTheme: const SyntaxTheme(
  ...
  static final NeomageTheme darkSolarized = NeomageTheme(
    info: const ThemeInfo(
      name: 'dark-solarized',
    ...
    textTheme: NeomageTextTheme.defaults(color: const Color(0xFF839496)),
    componentTheme: const NeomageComponentTheme(),
    syntaxTheme: const SyntaxTheme(
  ...
  /// Gruvbox dark theme.
  static final NeomageTheme darkGruvbox = NeomageTheme(
    info: const ThemeInfo(
      name: 'dark-gruvbox',
    ...
    textTheme: NeomageTextTheme.defaults(color: const Color(0xFFEBDBB2)),
    componentTheme: const NeomageComponentTheme(),
    syntaxTheme: const SyntaxTheme(
  ...
  /// Dracula dark theme.
  static final NeomageTheme darkDracula = NeomageTheme(
    info: const ThemeInfo(
      name: 'dark-dracula',
      displayName: 'Dracula',
      isDark: true,
  ...
  /// Default light theme.
  static final NeomageTheme lightDefault = NeomageTheme(
    info: const ThemeInfo(
      name: 'light-default',
    ...
    textTheme: NeomageTextTheme.defaults(color: const Color(0xFF1F2328)),
    componentTheme: const NeomageComponentTheme(),
    syntaxTheme: const SyntaxTheme(
  ...
  static final NeomageTheme lightSolarized = NeomageTheme(
    info: const ThemeInfo(
    ...
    textTheme: NeomageTextTheme.defaults(color: const Color(0xFF657B83)),
    componentTheme: const NeomageComponentTheme(),
    syntaxTheme: const SyntaxTheme(
      keyword: Color(0xFF859900),
      string: Color(0xFF2AA198),
  ...
  static final NeomageTheme lightGithub = NeomageTheme(
    info: const ThemeInfo(
      name: 'light-github',
  ...
  static final Map<String, NeomageTheme> all = {
...
/// Neomage themes to Flutter [ThemeData].
class ThemeManager {
  ThemeManager._({NeomageTheme? initial})
    : _currentTheme = initial ?? NeomageThemes.darkDefault {
    _controller = StreamController<NeomageTheme>.broadcast();
  }
  ...
  factory ThemeManager.withTheme(NeomageTheme theme) =>
      ThemeManager._(initial: theme);

  NeomageTheme _currentTheme;
  late final StreamController<NeomageTheme> _controller;

  final Map<String, NeomageTheme> _customThemes = {};

  /// The currently active theme.
  NeomageTheme get currentTheme => _currentTheme;
  ...
  List<ThemeInfo> getAvailableThemes() {
    return [
      ...NeomageThemes.all.values.map((t) => t.info),
      ..._customThemes.values.map((t) => t.info),
    ];
  }

  /// Convert a [NeomageColorScheme] to a Flutter [ThemeData].
  static ThemeData toMaterialTheme(NeomageColorScheme colors) {
  ...
  static bool _looksLikeDark(NeomageColorScheme colors) {
    final bg = colors.background;
    final luminance = bg.computeLuminance();
    return luminance < 0.5;
  }

  static Brightness _brightnessOf(NeomageColorScheme colors) {
