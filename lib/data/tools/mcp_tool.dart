...
//   - MCPTool.ts: base MCP tool with passthrough schema, permission checks,
...
const String mcpToolBaseName = 'mcp';

/// Maximum result size before disk persistence (100K chars).
const int mcpMaxResultSizeChars = 100000;
...
/// MCP tool prompt.
const String mcpPrompt =
    'MCP tools are provided by external servers and have their own '
    'input schemas. Each tool is namespaced as mcp__serverName__toolName.';
...
/// Truncation marker that MCP servers may append to output.
...
// Returns isSearch=false, isRead=false for tools that should NOT collapse
// (e.g., send_message, create_*, update_*).
//
...
// so matching is keyed on the tool name alone after normalizing
// camelCase/kebab-case to snake_case. Unknown tool names don't collapse
...
/// Search tools -- tools that search for data across a service.
const Set<String> _searchTools = {
  // Slack
  'slack_search_public',
  ...
  // 'find_projects', -- already in Sentry
  'find_sections',
  'find_comments',
  ...
  'find_filters',
  // AWS
  // 'search_documentation', -- already in Linear
  'search_catalog',
  // Terraform
  'search_modules',
  'search_providers',
...
/// Read tools -- tools that fetch/read specific data from a service.
const Set<String> _readTools = {
  ...
  'get_panel_image',
  // PagerDuty
  // 'list_incidents', -- already in Grafana
  // 'get_incident', -- already in Grafana
...
///
/// This is the base tool that gets cloned and configured per-server in
/// the MCP client. The name, description, prompt, inputSchema, and call
/// method are all overridden when an MCP server registers its tools.
///
/// Features:
...
class MCPTool extends Tool {
  ...
  @override
  Map<String, dynamic>? get mcpInfo => _serverInfo?.toMap();

  @override
  int? get maxResultSizeChars => mcpMaxResultSizeChars;

  @override
  String get userFacingName => _userFacingName;
  ...
  Future<PermissionDecision> checkPermissions(
    ...
    // MCP tools require passthrough permission check -- the MCP client
    // handles the actual permission logic.
    return const AskDecision(
      PermissionAskDecision(message: 'MCPTool requires permission.'),
    );
  }
  ...
  bool isResultTruncated(String output) {
    return isOutputLineTruncated(output);
  }

  /// Get the search/read classification for this tool.
  McpToolClassification get classification {
    final serverName = _serverInfo?.serverName ?? '';
    return classifyMcpToolForCollapse(serverName, _name);
  }
